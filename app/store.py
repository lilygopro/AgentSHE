...
def list_sessions(owner_telegram_id: int | None = None) -> list[dict[str, Any]]:
    with locked_state() as state:
        ...
        for s in state["sessions"].values():
            if owner_telegram_id is not None and s.get("owner_telegram_id") != owner_telegram_id:
                continue
...
def connect_commands(user_id: int, base_url: str | None = None) -> dict[str, str]:
    """One-shot install command; session runs on the PC.

    Bootstrap script is still served by the bot (tiny, embeds enroll + bot_base),
    but HelperHost binaries are downloaded from GitHub Releases (fixed URL).
    """
    ...
    return {
        "enroll_key": key,
        "base_url": base,
        "github_release": config.GITHUB_RELEASE_BASE,
        "mac": f"curl -fsSL '{base}/agent?action=bootstrap-sh&enroll={key}' | bash",
        "windows": f"irm '{base}/agent?action=bootstrap&enroll={key}' | iex",
    }
